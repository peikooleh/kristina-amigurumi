...
(async function initHeroSlideshow(){
  ...
  const track = root.querySelector('.hero-slideshow__track');
  const dots  = root.querySelector('.hero-slideshow__dots');

  // helper: check if a url exists quickly
  ...
  // scan sequential files until a few misses in a row
  const exts = ['webp','jpg','jpeg','png','avif'];
  const base = 'photos/slideshow/';
  const found = [];
  let misses = 0;
  for (let i=1; i<=200 && misses<5; i++){
    let ok = false;
    for (const ext of exts){
      const url = `${base}${pad(i)}.${ext}`;
    ...
    if (!ok) misses++; else misses = 0;
  }

  if (!found.length) return; // nothing to show

  // Build DOM
  found.forEach((src, idx) => {
    const fig = document.createElement('figure');
    fig.className = 'hero-slideshow__slide';
    fig.setAttribute('role', 'listitem');
    const img = new Image();
    img.src = src.replace(/\.jpe?g$/i, '.webp'); // optimistic webp
    img.onerror = () => { img.onerror = null; img.src = src; };
    img.decoding = 'async';
    img.loading = 'eager';
    img.alt = 'Игрушка ' + (idx+1);
    fig.appendChild(img);
